...
class BinarySearchTree(Tree):
    ...
    def diameter(self):
        ...
        def _diameter_helper(node):
            if self.is_leaf(node):
                return 0, 1

            left_diam, left_height = _diameter_helper(node._left) if node._left else (0, 0)
            right_diam, right_height = _diameter_helper(node._right) if node._right else (0, 0)

            return max(left_diam, right_diam, left_height + right_height), max(left_height, right_height) + 1

        return _diameter_helper(self._root)[0] if not self.is_leaf(self._root) else 0
